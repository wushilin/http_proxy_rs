...
async fn main() {
    ...
    let tower_service = tower::service_fn(move |req: Request<_>| {
        ...
        warn!("{} method is {}", req_id_clone, req.method());
        ...
        async move {
            ...
            if req.method() == Method::CONNECT {
                ...
                match target_host {
                    ...
                    Some(host) => {
                        let access = config_clone.check_access(&req_id, &host).await;
                        if access {
    ...
    let config_clone = config_clone.get().unwrap();
    let addr = format!(
        "{}:{}",
        config_clone.get_bind_address(),
        config_clone.get_bind_port()
    );
